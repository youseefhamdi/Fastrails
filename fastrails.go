...
func parseFlags() *Config {
	...
	pflag.IntVarP(&config.maxPages, "max-pages", "m", 100, "Maximum number of pages to scrape (default: 100)")
	pflag.DurationVar(&config.delay, "delay", 3400*time.Millisecond, "Delay between requests (default: 3400ms)")
	pflag.BoolVar(&config.silent, "silent", false, "Silent mode.")
	pflag.BoolVar(&config.versionFlag, "version", false, "Print the version of the tool and exit.")
...
/_/      \___,/ /_____) /_/  /_/  \_\\___,//____//____//_____)
 ...
 `
	...
	fmt.Printf("%s\n%60s\n\n", banner, "Current Fastrails version "+version)
}

func printVersion() {
	fmt.Printf("Current Fastrails version %s\n", version)
}
...
func processDomain(domain string, config *Config) {
	...
	emptyPageCount := 0
	maxEmptyPages := 3 // Allow 3 consecutive empty pages before stopping
	...
	for page := 1; page <= config.maxPages; page++ {
		...
		if resp.StatusCode == 401 || resp.StatusCode == 403 {
			resp.Body.Close()
			fmt.Fprintf(os.Stderr, "Authentication failed (HTTP %d): Cookie may be expired\n", resp.StatusCode)
			os.Exit(1)
		}
		...
		// Better detection for authentication issues vs empty results
		if len(matches) == 0 {
			emptyPageCount++
			if emptyPageCount >= maxEmptyPages {
				if config.verbose {
					fmt.Fprintf(os.Stderr, "Reached %d consecutive empty pages, stopping enumeration for %s\n", maxEmptyPages, domain)
				}
				return
			}
			// Check if the page contains authentication error messages
			if strings.Contains(string(body), "login") || strings.Contains(string(body), "sign in") {
				fmt.Fprintf(os.Stderr, "Cookie expired or authentication required\n")
				os.Exit(1)
			}
			continue
		}

		// Reset empty page counter on successful match
		emptyPageCount = 0

		// Check if we only found the domain itself (no subdomains)
		if len(matches) == 1 {
			subdomain := matches[0][1]
			if subdomain == domain {
				if config.verbose {
					fmt.Fprintf(os.Stderr, "No subdomains found for %s\n", domain)
				}
				return
			}
		}

		// Print subdomains, skipping the first line (which is usually the domain itself)
	...
	if config.verbose {
		fmt.Fprintf(os.Stderr, "Finished processing %s (reached max pages: %d)\n", domain, config.maxPages)
	}
}
...
func setHeaders(req *http.Request, config *Config) {
	...
	req.Header.Set("Sec-Ch-Ua", `"Chromium";v="130", "Not?A_Brand";v="99"`)
